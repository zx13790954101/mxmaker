      ...
      <ul>
        <li v-for="(item,index) in goodsList" :key="item.addressId">
            <div class="top-content">
              ...
              <p class="address"><span>地址： {{item.province!=item.city?item.province:''}} </span><span>{{item.city}}</span><span>{{item.area}}{{item.address}}</span></p>
            </div>
...
<script>
    ...
    export default{
      ...
      methods:{
        toBack:function(){
          bus.$emit('userpage','edit-address')
        },
        handleChange(id) {
          ...
          that.$http.get(globalPath+'/GetAddressList', {
            ...
            if (typeof(res.body) != 'object') {
              ...
              $(data2).each(function (index,element) {
                  console.log('province的地址的详细',element);
        ...
        checkedChange(index){
          ...
          if(event.target.checked){
            ...
            if(that.checkDefalut!='N') {
              console.log('sssss1',that.checkDefalut);
            that.cancelCheckedY(that.checkDefalut,false);
              console.log('sssss2',index);
            }
