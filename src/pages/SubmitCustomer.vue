  ...
  <div class="shopping-cart">
    ...
    <div  class="main">
      <ul>
        <li v-for="(item,index) in goodsList" :key="item.addressId">
          <div class="top-content" @click="toSubmitOrder(index)">
    ...
    <footer>
      <div class="buttom-address">
        <el-button @click="toAddress" ><i class="iconfont icon-jia"></i>&nbsp;添加地址</el-button>
      </div>
    </footer>
  </div>
</template>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      toAddress:function(){
        sessionStorage.setItem('submitCustomer','1');
        ...
        that.$http.get(globalPath+'/GetAddressList', {
          ...
          if (typeof(res.body) != 'object') {
            ...
            $(data2).each(function (index,element) {
              ...
              $(address).each(function (index,element2) {

                $(element2.city).each(function (index,element3) {
                  if(element3.name==element.city){
                    element.province=element2.name;
                    if( element.province==element.city){
                      element.province='0';
                    }
                  }
                })
              });
              that.goodsList=data2;
      ...
      toSubmitOrder:function (index) {
        localStorage.setItem('selectAddress',JSON.stringify(this.goodsList[index]));
        bus.$emit('curPage','submit-order');
      }
    }
  }
