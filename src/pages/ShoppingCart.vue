      ...
      <ul>
        <li v-for="(item,index) in goodList" :key="item.id">
          <div>
            ...
            <div class="right">
              <el-input-number v-model="item.goodNumer" @change="handleChange(index,item.goodNumer)" :min="1" :max="50" class="buy_num"></el-input-number>
  ...
  export default {
    ...
    methods:{
      ...
      handleChange(index,num) {
        var that=this;
        that.goodList[index].goodNumer=num;
        localStorage.setItem("addShopcart_Array",JSON.stringify(that.goodList));
      },
      init:function(){
        var that=this;
      ...
      deleteAll: function(){
        ...
        that.goodList=JSON.parse(localStorage.getItem("addShopcart_Array"));
        that.goodList.length
          bus.$emit('shopcartNum', that.goodList.length);
      },
      ...
      settlement: function(){
        ...
        var  localGoodinfo=[];
        $(that.goodList).each(function (index,element) {
          $(that.checkedList).each(function (checkedIndex,element2) {
            if(element2.id==element.id){
              goodInfo.push(element);
            }
          });
        });
        var data={
